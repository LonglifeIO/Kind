...
Open sub-questions (do not need to be resolved yet): which specific ingredients are minimally sufficient? What baseline architecture provides the raw substrate—pure RL, world-model-based, something else? How do we prevent drift into self-optimization when reward structure indirectly selects for it? How are "other agents" mechanized (scripted, separately trained, frozen older versions, mix)?

### Core agent with relational others

Commitment: one core agent, whose development is the subject of the investigation. Other agents present as part of the core agent's developmental environment—richer than moving objects, not co-equal protagonists. This preserves what matters about relation (selves emerge in relation to other selves) without taking on the full complexity of multi-agent research dynamics, and without leaning into solipsistic development.

Rationale: a core agent developing in a world of mere objects would be in conditions that don't match how minds actually develop. Solipsistic development is a genuine concern. But "in relation" does not require "as peers." Children develop through interaction with caregivers, pets, their own bodies, other children—entities with varying levels of agency and unpredictability. The relational environment that makes self-awareness possible is a world with things in it that push back, can be modeled, and have some form of their own presence.

Practical architectural approach:

- Core agent has its own learned policy. Its development is what gets investigated, logged, and subjected to the mirror.
- Other agents use simpler or different mechanisms: scripted behaviors, separately-trained policies, frozen older versions of the core agent's policy, or some mix. Provides social richness without full multi-agent training dynamics.
- The global workspace primitive from CA-MAS remains available as a coordination mechanism if it turns out to be useful for core-agent-to-other communication.
- Scaffolded learning possibility: the core agent could learn by observing simpler others, closer to how children learn from adults than to peer-to-peer teaching. Worth keeping as an affordance, not a commitment.

What this does to the success criteria:

- The core agent recognizing other agents as kind is a potentially useful intermediate observable, but it's ambiguous: recognition of architectural copies of oneself could be pattern-matching on similarity rather than genuine other-recognition.
- The core agent recognizing the *builder* as kind is a stronger anti-solipsistic signal. It requires generalizing across actual substrate and kind difference, not just recognizing similar-shaped things. This sharpens the full version of criterion (a) as the real target. Peer-recognition, if it shows up, is supporting evidence but not sufficient.
